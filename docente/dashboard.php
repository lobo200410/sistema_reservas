    ...
    tailwind.config = {
      theme: {
        extend: {
          ...
          boxShadow: {
            soft: '0 12px 32px rgba(16,24,40,.07)'
          },
          borderRadius: {
            xl2: '1rem'
          }
...
<body class="min-h-screen bg-gray-50 text-gray-900">
    ...
    <aside id="sidebar" class="fixed inset-y-0 left-0 z-40 w-72 -translate-x-full md:translate-x-0 transition-transform duration-200 bg-white border-r border-gray-200 shadow-soft">
      <div class="h-full flex flex-col">
        ...
        <nav class="flex-1 px-3 py-4 text-sm space-y-1">
          ...
          <a href="#" onclick="event.preventDefault(); cargarContenido('mis_materias')"
             class="group flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100">
            <span class="w-9 h-9 rounded-lg bg-gray-100 grid place-items-center text-gray-600 group-hover:bg-white group-hover:shadow">
          ...
          <a href="#" onclick="event.preventDefault(); cargarContenido('mis_reservas')"
             class="group flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100">
            <span class="w-9 h-9 rounded-lg bg-gray-100 grid place-items-center text-gray-600 group-hover:bg-white group-hover:shadow">
          ...
          <p class="px-3 pt-3 pb-1 text-[11px] uppercase tracking-wider text-gray-400">Soporte</p>

          <a href="#" onclick="event.preventDefault(); cargarContenido('soporte')"
             class="group flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-100">
            <span class="w-9 h-9 rounded-lg bg-gray-100 grid place-items-center text-gray-600 group-hover:bg-white group-hover:shadow">
    ...
    <main class="flex-1 md:ml-72 min-w-0">
      ...
      <section class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        ...
        <div class="py-6 text-center text-xs text-gray-400">
          © <?= date('Y'); ?> Sala de Video — Universidad
        </div>
      </section>
    </main>
  </div>
  ...
  <script>
    ...
    function openSidebar(){ sidebar.classList.remove('-translate-x-full'); if (backdrop) backdrop.classList.remove('hidden'); }
    function closeSidebar(){ sidebar.classList.add('-translate-x-full');   if (backdrop) backdrop.classList.add('hidden'); }
    ...
    window.cargarContenido = (function(orig){
      return function(v){
        const map = { inicio:'Inicio', mis_materias:'Mis Materias', mis_reservas:'Mis Reservas', soporte:'Soporte', reserva:'Nueva Reserva' };
        const crumb = document.getElementById('crumb');
        if (crumb && map[v]) crumb.textContent = map[v];
        orig(v);
      }
    })(window.cargarContenido || function(){});
  </script>
</body>
</html>
